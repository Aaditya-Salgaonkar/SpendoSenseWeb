...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1c] text-white flex flex-col items-center px-6 md:px-16">
      {/* Sticky Navbar */}
      <div className="w-full fixed top-0 left-0 bg-[#0a0f1c] z-50">
        <Navbar />
      </div>
      ...
      <motion.header 
        ...
        className="w-full max-w-6xl text-center pt-24 pb-12"
      >
      ...
      <footer className="w-full max-w-5xl text-center mt-16 mb-12">
        ...
        {!subscribed ? (
          <Button className="mt-4 px-6 py-3 text-lg bg-purple-600 hover:bg-purple-700" onClick={() => setSubscribed(true)}>
            Subscribe Now
          </Button>
        ) : (
          <motion.div className="mt-4 text-xl font-semibold text-green-400" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1 }}>
